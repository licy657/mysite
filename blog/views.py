...
def get_blogs_common_data(request,blogs_all_list):
    ...
    paginator = Paginator(blogs_all_list, ST.EACH_PAGE_BLOGS_NUMBER)  # 每2进行分页
    page_num = request.GET.get('page', 1)  # 获取url的页面参数（GET请求）
    page_of_blogs = paginator.get_page(page_num)
    current_page_num = page_of_blogs.number  # 获取当前页码
    # 获取当前页码范围【-2, 2】
    page_range = [current_page_num + i for i in range(-2, 3) if 0 < current_page_num + i <= paginator.num_pages]
    # 加上省略页码标记
    if page_range[0] - 1 > 2:
        page_range.insert(0, '...')
